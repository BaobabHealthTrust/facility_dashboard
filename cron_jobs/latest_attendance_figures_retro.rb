  ...
  def print_result(request)
    ...
    result = {
      ...
      attendance_figures = con.query("SELECT count(person_id) number_of_patients,DATE(obs_datetime)
        ...
        AND voided = 0 AND DATE(obs_datetime) = DATE('#{date}') GROUP BY value_text, DATE(obs_datetime);")

      #people = con.query("SELECT WHERE patient_id IN (SELECT DISTINCT patient_id FROM encounter WHERE encounter_datetime = DATE('#{date}'))")
    ...
    attendance_figures = con.query("SELECT COUNT(DISTINCT patient_id) number_of_patients,
                            ...
                            FROM encounter WHERE voided = 0 AND DATE(encounter_datetime) = DATE('#{date}')
    ...
    total_admission = con.query("SELECT count(*) FROM encounter WHERE DATE(encounter_datetime) = DATE('#{date}') AND voided = 0 AND
                                  encounter_type = #{admit_enc_type}").fetch_row[0]

    total_discharge = con.query("SELECT count(*) FROM encounter WHERE DATE(encounter_datetime) = DATE('#{date}') AND voided = 0 AND
    ...
    admitted_patients = con.query("SELECT COUNT(distinct patient_id) FROM patient_state as s inner join patient_program as p
                                   ...
                                   WHERE state = #{admitted_state} 
										AND DATE(s.start_date) <= DATE('#{date}')
										AND (s.end_date IS NULL OR DATE(s.end_date) > DATE('#{date}'))
										AND p.voided = 0 AND s.voided = 0").fetch_row[0]
    ...
    admissions = con.query("SELECT count(*), value_text, DATE(obs_datetime) FROM obs WHERE DATE(obs_datetime)= DATE('#{date}')
    ...
    discharge_by_gender =  con.query("SELECT gender, COUNT(person_id) FROM person WHERE person_id IN (SELECT patient_id FROM encounter
                                      WHERE encounter_type = #{discharge_concept} AND DATE(encounter_datetime) = DATE('#{date}') AND voided = 0)
                                      GROUP BY gender ")
